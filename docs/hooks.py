...
def _copy_readme_assets() -> None:
    # README.md references images via paths relative to the repo root (e.g. assets/images/...).
    # Mirror that same layout under docs/ so the copied docs/index.md resolves them correctly.
    assets_src = REPO_ROOT / "assets"
    assets_dst = ASSETS_DIR
    if assets_src.exists():
        shutil.copytree(assets_src, assets_dst, dirs_exist_ok=True)
        log.info("Copied assets/ → docs/assets/")
    else:
        log.warning("assets/ directory not found, skipping README asset copy")
