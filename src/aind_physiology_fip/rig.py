...
class FipCamera(rig.cameras.SpinnakerCamera):
    ...
    color_processing: Literal["Default", "NoColorProcessing"] = Field(default="Default", description="Color processing") #TODO
    ...
    gamma: Optional[float] = Field(default=None, ge=0, description="Gamma. If None, will disable gamma correction.") #TODO
    adc_bit_depth: Literal[rig.cameras.SpinnakerCameraAdcBitDepth.ADC12BIT] = Field(
        default=rig.cameras.SpinnakerCameraAdcBitDepth.ADC12BIT, description="ADC bit depth. If None will be left as default."
    )
    pixel_format: Optional[rig.cameras.SpinnakerCameraPixelFormat] = Field(
        default=rig.cameras.SpinnakerCameraPixelFormat.MONO16, description="Pixel format. If None will be left as default."
    )
    region_of_interest: rig.cameras.Rect = Field(default=rig.cameras.Rect(), description="Region of interest", validate_default=True)
    video_writer: Optional[VideoMatrixWriter] = Field(
        default=VideoMatrixWriter(), description="Video writer. If not provided, no video will be saved.", validate_default=True
...
class HarpCuttlefishFip(rig._harp_gen._HarpDeviceBase):
    ...
    additional_settings: HarpCuttlefishFipSettings = Field(description="Additional settings for the cuTTLefishFip device")
...
class AindPhysioFipRig(rig.AindBehaviorRigModel):
    ...
    roi_settings: List[RoiSettings] = Field(default=[], title="Region of interest settings", description="Region of interest settings")
    cuttlefish_fip: HarpCuttlefishFip = Field(title="CuttlefishFip", description="CuttlefishFip board for controlling the trigger of cameras and light-sources")